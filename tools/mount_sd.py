...
#
# IMPORTANT: a plain `mpremote connect <port> ...` does an implicit
# soft-reset before running its command, which unmounts /sd immediately
# -- so deploy.py's FIRST command after this (the per-game cleanup, then
# the actual copy) must use `mpremote connect <port> resume <command>`,
# NOT another plain connect, or the mount this script just set up is
# gone before the next command even starts (confirmed on hardware: the
# fake-mount bug recurring right after a successful mount here). A
# tried-and-abandoned alternative -- chaining mount + cleanup + copy into
# one `run <script> + cp ...` invocation to dodge this -- hung
# indefinitely on real hardware instead; don't reintroduce that.

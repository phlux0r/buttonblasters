...
- Star Bonk (`games/bonk`) — on real hardware now, hit the StripBufferPool MemoryError twice (see entry above): first from a load() allocation-order bug (fixed), then again from the pool simply being too big for this heap's typical fragmentation (fixed by halving STRIP_H, 150KB→75KB pool). Neither fix is bench-confirmed yet. Still blocking before fully trustworthy: re-test that the pool now seats reliably, bake the 10 required assets (see asset spec table above — board asset wasn't baked yet as of either MemoryError report, so gameplay was still running on the `_FlatBackground` placeholder, now at strip_h=16 not 32), and confirm the first-ever StripRenderer hardware wiring (CS/DC pins, blocking transmit) actually paints correctly.

...
| `bonk/bg_bonk_480x320.bz` | **LE**, kind 0, `strip_h=16` | Main board — can be fully illustrated everywhere now (no flat-colour play-zone constraint) |
| `menu/bgm_menu-bonk_480x320.bz` | BE, kind 1 | Menu card (optional — `core/menu.py` falls back to a procedural card if missing) |

`strip_h` on the board **must** be 16 to match `drivers.strip_renderer.STRIP_H` (the single source of truth — `core/sprite_engine.py` imports it rather than defining its own copy, after those two drifted out of sync once already) — `SpriteEngine.set_background()` raises if it doesn't. Was 32 originally; see the StripBufferPool MemoryError entry below for why it's 16 now. Bake this asset at `strip_h=16` — it was never baked at 32 in the first place, so this isn't a re-bake, just the number to use. `tools/deploy.py` now splits Tier A (`sprb_`/`spr_`, small, permanent littlefs) from Tier B (`bgm_`/`bg_`, large, SD-installed at game load) by filename prefix for any game folder, not just Match's — Bonk needed no deploy.py changes beyond that generalization.
...
- **75KB `StripBufferPool` (was 150KB), held for the whole game via `MainScreenAdapter.open()`/`close()`.** **Confirmed failing on real hardware, twice.** First: `MemoryError: ... rgb666[1] did not seat (free=205552, largest~needs 46080 contiguous)` — traced to `games/bonk/game.py`'s `load()` allocating the 20KB legend arena *before* `adapter.open()`, violating this codebase's "acquire hardest-first" rule; fixed by reordering. Second, *after* that fix: `MemoryError: ... rgb666[1] did not seat (free=211376, largest~needs 46080 contiguous)` — same failure, confirming the fragmentation predates Bonk's `load()` entirely (menu rendering, boot sequence, or prior session state), not something reorderable within one game's load. Fixed by halving `STRIP_H` 32→16 (`drivers/strip_renderer.py`, now the single source of truth for both it and `core/sprite_engine.py`), which halves the largest single contiguous block the pool needs (45KB→22.5KB) — a smaller ask is far more likely to fit into whatever gaps a non-compacting allocator has left, regardless of the exact fragmentation pattern. Cost: ~2x the strip count per repaint (10→20 strips for a full 320-row screen), not yet bench-measured for actual timing impact. **Re-test on hardware before trusting this is resolved** — same category of "reasoned, not confirmed" as everything else in this section.
...
- Star Bonk (`games/bonk`) — on real hardware now, has hit three confirmed MemoryErrors across `load()` and `run()` (see entries above): two StripBufferPool seating failures (allocation order, then pool size) and one mid-gameplay audio-driven fragmentation crash. All three have fixes applied, none bench-confirmed yet — the board asset wasn't baked yet, or was baked at the wrong `strip_h`, as of every report so far, so gameplay was still running on the `_FlatBackground` placeholder throughout. Still blocking before fully trustworthy: re-bake `bg_bonk_480x320.bz` at `strip_h=16`, re-test a full playthrough (all 3 rounds, enough hits to trigger the speed-up curve) for both memory fixes, and confirm the first-ever StripRenderer hardware wiring (CS/DC pins, blocking transmit) actually paints correctly once real art is in the loop.

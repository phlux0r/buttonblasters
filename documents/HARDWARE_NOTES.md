...
- Star Bonk (`games/bonk`) — on real hardware now, has hit five confirmed memory/allocation bugs (see entries above): three StripBufferPool seating MemoryErrors at three different pool sizes (32→16→8), one mid-gameplay audio-driven fragmentation MemoryError, and one button-legend-arena MemoryError on a second play session (fixed by making that arena persistent). All have fixes applied, none bench-confirmed yet. Also fixed from user playtesting: screen tearing when a hit's sound played (fire-and-forget audio racing an unlocked SPI0 render — see the StripRenderer entry above), `MAX_SCORE` miscalibrated so stars looked stuck regardless of skill (was worst-case 96, now average-case-perfect 60), only-last-attempt-counts when using "Play Again" (now tracks a session-best), round-announcement text size, intermittent/laggy touch target detection (was discrete release-based `TOUCH_TAP`, now live `touch_down`/`touch_pos` polling), and legend tile background colour (now white, matching Match It!'s icon convention). The board asset wasn't baked yet, or was baked at the wrong `strip_h`, as of every report so far, so gameplay was still running on the `_FlatBackground` placeholder throughout — none of this has been tested with the real board art in the loop. Still blocking before fully trustworthy: re-bake `bg_bonk_480x320.bz` at `strip_h=8`, re-test a full playthrough (all 3 rounds, enough hits to trigger the speed-up curve, from an actual fresh power-on if testing the memory fixes — a USB replug alone doesn't reset a battery-powered session), a *second* play session in the same power-on run (to confirm the legend-arena fix and check whether the strip pool itself now surfaces as the next fragmentation victim), and confirm the first-ever StripRenderer hardware wiring (CS/DC pins, blocking transmit) actually paints correctly once real art is in the loop.

...
- Native working window is **240×300** (portrait), not the datasheet's 240×280 — 300 rows fills the full physical screen. **Landscape (current, for the 2×2 shell layout): addressable 300×240, but the true VISIBLE glass is only native columns 20..299 (280 wide)** — `tests/test_15_button_landscape.py`'s original "300×240 confirmed clean" was wrong: a full-screen solid colour fill can't reveal a column offset (the panel just silently ignores out-of-range/off-glass columns, so it still looks clean edge-to-edge). Ruler-measured precisely on 2026-09-06 with `tests/test_16_button_edge_probe.py` (10px-spaced tick marks): 28/30 top ticks visible, first visible one flush at x=20; 23/24 left ticks visible, the one miss (x=0,y=0) explained by top-left corner rounding, not a separate row crop. **Fix: `config.BTN_COL_OFFSET = 20`, added to every x0/x1 in `ST7789._set_window()` (drivers/display.py), plus `config.BTN_W = 280`** — games/menu code keeps using logical 0-based coordinates against `BTN_W`/`BTN_H` as before; the shift happens once, centrally, not at every call site. An earlier attempt at just narrowing `BTN_W` to 280 with **no** offset made things visibly worse (a fill no longer covered the true window at all) — width and offset both matter, neither alone is sufficient. No evidence of any row/Y offset — this is X-only. RGB colour order was already correct at MADCTL bit3=0 in portrait — the landscape values keep that bit, don't add BGR when experimenting further. **MADCTL is per-button, not a single value** — see the dedicated subsection below (2×2 mounting rotates BTN-2/3 180° from BTN-0/1); the column offset is confirmed uniform across all 4 buttons regardless of MADCTL, so it's applied once in `_set_window()`, not per-button.
...
- **Switching orientation (or correcting a wrong width, as above) invalidates every full-screen button asset baked at the old dimensions** — the 96×96 icon sprites (Match's `sprb_*`, Bonk's `spr_*`/`sprb_*`) are unaffected since they're centered by a formula that recomputes from `config.BTN_W`/`BTN_H`, but anything baked at the whole button canvas (menu tiles, prev/next/back/again tiles) needs re-baking at the corrected 280×240 and its filename updated accordingly (`_300x240.bz` → `_280x240.bz`). Path constants (`core/menu.py`'s `_MENU_TILE`/`BTN_PREV_PATH`/`BTN_NEXT_PATH`, each `game.py`'s `*_TILE_PATH`) were updated to the `_280x240` names on 2026-09-06 — **the actual .bz files still need re-baking from source art at the corrected canvas**; until that's done, `paint_btn_bg()` won't find them at the new paths and falls back to each game's procedural placeholder (safe, just plainer).

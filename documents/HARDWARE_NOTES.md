...
- Native working window (portrait bring-up) was bench-measured at **240×300**, not the datasheet's 240×280 — 300 rows fills the full physical screen. **Landscape (current, for the 2×2 shell layout): 280×240**, datasheet-confirmed — **not 300×240**. `tests/test_15_button_landscape.py` originally "confirmed" 300×240 clean via a full-screen solid colour fill, but that check can't catch a too-wide window: the panel silently drops column addresses past its true visible edge rather than showing a dead/garbage strip, so the wrong width still looked clean edge-to-edge. Only surfaced once something was drawn as a border flush against the far edge (Button Memory's persistent lit-button border — Match It!/Star Bonk!'s borders are brief win/lose flashes, easy to miss a clipped edge in). `config.BTN_W` corrected 300→280 on 2026-09-06; **every button-screen asset baked at 300×240 needs re-baking at 280×240** (see the note below). Note this doesn't reconcile with the 240×300 portrait figure (240×300 = 72000px vs 280×240 = 67200px, not the same total, despite both being independently bench/datasheet-confirmed) — an open question if it ever matters again, e.g. if portrait is revisited. RGB colour order was already correct at MADCTL bit3=0 in portrait — the landscape values keep that bit, don't add BGR when experimenting further. **MADCTL is per-button, not a single value** — see the dedicated subsection below (2×2 mounting rotates BTN-2/3 180° from BTN-0/1); the width bug was confirmed uniform across all 4 buttons regardless of MADCTL, so it's not a rotation/coordinate bug, purely a wrong constant.
...
- **Switching orientation (or fixing a wrong dimension, as above) invalidates every full-screen button asset baked at the old dimensions** — the 96×96 icon sprites (Match's `sprb_*`, Bonk's `spr_*`/`sprb_*`) are unaffected since they're centered by a formula that recomputes from `config.BTN_W`/`BTN_H`, but anything baked at the whole button canvas (menu tiles, prev/next/back/again tiles) needs re-baking at the corrected 280×240 and its filename updated accordingly (`_300x240.bz` → `_280x240.bz`).
- **RESOLVED** (was open as of 2026-09-06, now explained above): Button Memory's persistent lit-button border was found clipped on its left edge, confirmed uniform across all 4 buttons regardless of MADCTL (ruling out a rotation/coordinate bug tied to BTN-2/3's flipped `0x60` vs BTN-0/1's `0xA0`). Root cause was the 300×240 vs true 280×240 width, not a physical bezel crop as first suspected — the border's right edge was being computed 20px further right than the panel's real edge, and since the panel silently drops out-of-range columns instead of showing garbage, only edge-flush content (a border) revealed it, never a centered icon or a full-screen fill. `games/memory/game.py`'s `BORDER_INSET=0`/`BORDER_LEFT_EXTRA=10`/`BORDER_THICKNESS=24` were tuned blind against the wrong width and should be re-tested now that `config.BTN_W` is correct — likely `BORDER_LEFT_EXTRA` can drop back to 0.

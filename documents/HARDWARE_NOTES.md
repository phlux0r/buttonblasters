...
- **150KB `StripBufferPool`, held for the whole game via `MainScreenAdapter.open()`/`close()`.** **Confirmed failing on real hardware** (not hypothetical): `MemoryError: StripBufferPool: rgb666[1] did not seat (free=205552, largest~needs 46080 contiguous)` — ~200KB free overall, but not 45KB *contiguous* after the first RGB666 buffer took its slice. Root cause: `games/bonk/game.py`'s `load()` was allocating the 20KB legend arena *before* `adapter.open()`, violating this codebase's own "acquire hardest-first" rule (see `drivers/strip_renderer.py`'s docstring) — MicroPython's GC here doesn't compact/move live objects, so a smaller allocation landing first can split what would've been a large contiguous run, and no amount of `gc.collect()` afterward undoes that. Fixed by reordering `load()` so `adapter.open()` runs first, before any other new heap allocation. **Not guaranteed to fully resolve it** — reordering removes one identified contributor, but the actual largest-contiguous-block size at that point in the session depends on the whole allocation history since boot (menu rendering, prior games, Tier B installs), which wasn't fully characterized. If it still fails after reordering, the next lever is shrinking `STRIP_H` (32 → 16 halves each RGB666/RGB565 buffer to ~22.5KB/~15KB, at the cost of twice the render passes per full-screen repaint).
...
- Star Bonk (`games/bonk`) — on real hardware now, hit the StripBufferPool MemoryError documented above on first attempt; reordered `load()` to fix the identified cause but that reorder itself is not yet re-confirmed on hardware. Still blocking before fully trustworthy: bake the 10 required assets (see asset spec table above — board asset wasn't baked yet as of the MemoryError report, so gameplay was still running on the `_FlatBackground` placeholder), re-test that the pool now seats reliably, and confirm the first-ever StripRenderer hardware wiring (CS/DC pins, blocking transmit) actually paints correctly.
